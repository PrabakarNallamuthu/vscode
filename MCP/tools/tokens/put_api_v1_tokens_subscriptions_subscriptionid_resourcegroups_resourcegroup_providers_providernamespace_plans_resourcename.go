...
func Put_api_v1_tokens_subscriptions_subscriptionid_resourcegroups_resourcegroup_providers_providernamespace_plans_resourcenameHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		providerNamespaceVal, ok := args["providerNamespace"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: providerNamespace"), nil
		}
		providerNamespace, ok := providerNamespaceVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: providerNamespace"), nil
		}
		...
		url := fmt.Sprintf("%s/api/v1/Tokens/subscriptions/%s/resourceGroups/%s/providers/%s/plans/%s", cfg.BaseURL, subscriptionId, resourceGroup, providerNamespace, resourceName)
...
func CreatePut_api_v1_tokens_subscriptions_subscriptionid_resourcegroups_resourcegroup_providers_providernamespace_plans_resourcenameTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_api_v1_Tokens_subscriptions_subscriptionId_resourceGroups_resourceGroup_providers_providerNamespace_plans_resourceName",
		mcp.WithDescription(""),
		mcp.WithString("subscriptionId", mcp.Required(), mcp.Description("")),
		mcp.WithString("resourceGroup", mcp.Required(), mcp.Description("")),
		mcp.WithString("providerNamespace", mcp.Required(), mcp.Description("")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_api_v1_tokens_subscriptions_subscriptionid_resourcegroups_resourcegroup_providers_providernamespace_plans_resourcenameHandler(cfg),
	}
}

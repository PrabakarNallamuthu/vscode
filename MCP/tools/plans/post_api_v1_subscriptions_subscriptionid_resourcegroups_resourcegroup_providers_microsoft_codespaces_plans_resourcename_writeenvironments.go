...
func Post_api_v1_subscriptions_subscriptionid_resourcegroups_resourcegroup_providers_microsoft_codespaces_plans_resourcename_writeenvironmentsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Codespaces/plans/%s/writeEnvironments%s", cfg.BaseURL, subscriptionId, resourceGroup, resourceName, queryString)
...
func CreatePost_api_v1_subscriptions_subscriptionid_resourcegroups_resourcegroup_providers_microsoft_codespaces_plans_resourcename_writeenvironmentsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_api_v1_subscriptions_subscriptionId_resourceGroups_resourceGroup_providers_Microsoft.Codespaces_plans_resourceName_writeEnvironments",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_api_v1_subscriptions_subscriptionid_resourcegroups_resourcegroup_providers_microsoft_codespaces_plans_resourcename_writeenvironmentsHandler(cfg),
	}
}

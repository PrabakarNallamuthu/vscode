...
func Post_api_v1_subscriptions_subscriptionid_resourcegroup_providers_microsoft_vsonline_plans_resourcename_deletedelegatesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/subscriptions/%s/%s/providers/Microsoft.VSOnline/plans/%s/deleteDelegates", cfg.BaseURL, subscriptionId, resourceGroup, resourceName)
...
func CreatePost_api_v1_subscriptions_subscriptionid_resourcegroup_providers_microsoft_vsonline_plans_resourcename_deletedelegatesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_api_v1_subscriptions_subscriptionId_resourceGroup_providers_Microsoft.VSOnline_plans_resourceName_deleteDelegates",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_api_v1_subscriptions_subscriptionid_resourcegroup_providers_microsoft_vsonline_plans_resourcename_deletedelegatesHandler(cfg),
	}
}
